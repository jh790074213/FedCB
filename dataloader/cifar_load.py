...
from dataloader.datasets import CIFAR10_truncated, SVHN_truncated, CIFAR100_truncated
...
def load_skin_data(datadir, train_idxs, test_idxs):  # idxs相对所有data
    ...
    all_data_path = datadir + 'ISIC2018_Task3_Training_GroundTruth.csv'
    all_data_df = pd.read_csv(all_data_path)
    all_data_df = pd.concat([all_data_df['image'], all_data_df['dx']], axis=1)

    X_train, y_train, X_test, y_test = [], [], [], []
    train_df = all_data_df.iloc[train_idxs]
    test_df = all_data_df.iloc[test_idxs]

    train_names = all_data_df.iloc[train_idxs]['image'].values.astype(str).tolist()
    train_lab = all_data_df.iloc[train_idxs]['dx'].values.astype(str)
    test_names = all_data_df.iloc[test_idxs]['image'].values.astype(str).tolist()
...
# 计算每个客户端数据的统计信息：数据量的均值和方差 返回数据量
def record_net_data_stats(y_train, net_dataidx_map):
    ...
    for net_i, dataidx in net_dataidx_map.items():
        # unq：类别数组 unq_cnt：每个类别对于的数量 ,y_train[dataidx] 获得是对于数据索引的标签
    ...
    print('All Client mean:', np.mean(data_list))
    print('All Client std:', np.std(data_list))
    print('Client Data statistics: %s' % str(net_cls_counts))
...
# 加载原始数据
def partition_data_allnoniid(dataset, datadir, n_classes, train_idxs=None, test_idxs=None, partition="noniid",
                             n_parties=10,
                             beta=0.8, unsup_num=9):
    if dataset == 'cifar10':
        X_train, y_train, X_test, y_test = load_cifar10_data(datadir)
    elif dataset == 'SVHN':
        ...
        if partition == "homo" or partition == "iid":
            ...
            while min_size < min_require_size:
                ...
                for k in range(n_classes):
                    ...
                    np.random.shuffle(idx_k)
                    # 每个参与方从类别 k 中获取的样本数量的比例。
                    proportions = np.random.dirichlet(np.repeat(beta, n_parties))
                    # 限制客户端数据数量，len(idx_j) < N / n_parties则保留p反之为0
                    # proportions = np.array([
                    #     p if i < n_parties - unsup_num else max(p * (len(idx_j) < n_train / n_parties), 0.01)
                    #     for i, (p, idx_j) in enumerate(zip(proportions, idx_batch))
                    # ])
                    proportions = np.array([
                        p if i < n_parties - unsup_num else p * (len(idx_j) < n_train / n_parties)
                        for i, (p, idx_j) in enumerate(zip(proportions, idx_batch))
                    ])

                    proportions = proportions / proportions.sum()
                    # 按比例划分，proportion生成9个分割点
                    proportions = (np.cumsum(proportions) * len(idx_k)).astype(int)[:-1]
                    idx_batch = [idx_j + idx.tolist() for idx_j, idx in zip(idx_batch, np.split(idx_k, proportions))]
                    min_size = min([len(idx_j) for idx_j in idx_batch])
...
def get_dataloader(args, data_np, label_np, dataset_type, datadir, train_bs, is_labeled=None, data_idxs=None,
    ...
    if dataset_type == 'SVHN':
        ...
        assert pre_sz == 240 and input_sz == 224, 'Error: Wrong input size for 224*224 dataset'
    ...
    if not is_testing:
        # 监督学习
        if is_labeled:
            # RandomHorizontalFlip随机水平翻转图像，翻转的概率为 p=0.5
        ...
        # 无监督学习
            ...
            ds = dataset.CheXpertDataset(dataset_type, data_np, label_np, pre_sz, pre_sz,
                                         ...
                                         is_testing=False)
        # 基于ds数据集的数据加载器类
        dl = data.DataLoader(dataset=ds, batch_size=train_bs, drop_last=True, shuffle=True, num_workers=8)
    # 测试
    else:
        ...
        dl = data.DataLoader(dataset=ds, batch_size=train_bs, drop_last=True, shuffle=False, num_workers=8)
    return dl, ds
